...
from PySide6.QtWidgets import (
    QApplication, QCheckBox, QDialog, QFormLayout, QHBoxLayout, QHeaderView,
...
class OSCServerThread(threading.Thread):
    ...
    def on_heartbeat(self, address, *args):
        ip = None
        try:
            # python-osc handler does not expose source IP by default.
            # Matching is therefore done by optional device name in address or any heartbeat as fallback.
    ...
    def on_any(self, address, *args):
        # Accept /alive, /ipad/alive, etc. as optional heartbeat aliases.
        ...
        try:
            ...
            while self.running:
                ...
                if text.startswith("/heartbeat") or text.startswith("/alive") or text.startswith("/ipad/alive"):
                    ...
                    self.app_ref.event_queue.put(("heartbeat", {"name": name, "ip": addr[0], "address": text, "time": time.time()}))
        except Exception as e:
...
class SettingsWindow(QMainWindow):
    def __init__(self, app_ref):
        ...
        self.setWindowIcon(QIcon(resource_path("hb_green.ico")))
        self.resize(760, 520)
        self.build_ui()
    ...
    def build_ui(self):
        ...
        form.addRow("PC Alive Interval (sec)", self.alive_interval)
        layout.addLayout(form)

        self.send_pc_alive = QCheckBox("Send /pc/alive to Devices")
    ...
    def save_settings(self):
        ...
        QMessageBox.information(self, "Saved", "Settings saved and monitoring restarted.")
    ...
    def handle_heartbeat(self, payload):
        ...
        for dev in self.enabled_devices():
            if ip and dev.get("ip") == ip:
                ...
                matched.append(dev["name"])
        # Fallback: if there is only one enabled device, accept any heartbeat.
        if not matched and len(self.enabled_devices()) == 1:
            matched = [self.enabled_devices()[0]["name"]]
        for m in matched:
        ...
        for dev in self.enabled_devices():
            ...
            if self.previous_status.get(name) is None:
                ...
                if not online and self.config.data.get("show_disconnect_popup", True):
                    self.tray.showMessage(APP_NAME, f"{name} disconnected", QSystemTrayIcon.Warning, 3000)
                if not online and self.config.data.get("play_alert_sound", False):
                    QApplication.beep()

    def send_pc_alive(self):
        ...
        for dev in self.enabled_devices():
            try:
                client = SimpleUDPClient(dev["ip"], int(dev.get("port", 8000)))
                client.send_message("/pc/alive", 1)
            except Exception:
                pass
